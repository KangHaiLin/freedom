...
class NumberUtils:
    ...
    @classmethod
    def is_equal(cls, a: Union[float, decimal.Decimal, str],
                b: Union[float, decimal.Decimal, str],
                precision: int = 4) -> bool:
        """判断两个数值是否相等，指定精度"""
        precision_factor = decimal.Decimal(f"1e-{precision}")
        return abs(decimal.Decimal(str(a)) - decimal.Decimal(str(b))) < precision_factor

    @classmethod
    def calculate_fee(cls, amount: Union[float, decimal.Decimal],
                     fee_rate: float, min_fee: float = 0.0) -> decimal.Decimal:
        """计算手续费"""
        fee = cls.round_amount(decimal.Decimal(str(amount)) * decimal.Decimal(str(fee_rate)))
        return max(fee, cls.round_amount(min_fee))

    @classmethod
    def calculate_profit(cls, buy_price: Union[float, decimal.Decimal],
                        sell_price: Union[float, decimal.Decimal],
                        quantity: int,
                        fee_rate: float = 0.0003,
                        min_fee: float = 5.0,
                        stamp_duty_rate: float = 0.001) -> decimal.Decimal:
        """计算交易利润（扣除手续费和印花税）"""
    ...
    @classmethod
    def calculate_profit_ratio(cls, buy_price: Union[float, decimal.Decimal],
                              sell_price: Union[float, decimal.Decimal],
    ...
    @classmethod
    def safe_divide(cls, numerator: Union[float, decimal.Decimal],
                   denominator: Union[float, decimal.Decimal],
                   default: float = 0.0) -> decimal.Decimal:
        """安全除法，避免除零错误"""
        try:
