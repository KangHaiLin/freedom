...
class DataQualityMonitor(BaseMonitor):
    ...
    def __init__(self, name: str, config: Dict = None, storage_manager=None):
        ...
        configured_threshold = self.config.get("thresholds", {}).get(
        ...
        self.thresholds = {"overall_score": configured_threshold}
    ...
    def _check_accuracy(self, data: pd.DataFrame = None) -> Dict:
        ...
        if data is None:
            ...
            # 检查价格是否合理（支持price和close两种列名）
            for price_col in ["price", "close"]:
                if price_col in data.columns:
                    invalid_prices = data[price_col].apply(
                        lambda x: not (isinstance(x, (int, float)) and x > 0 and x < 10000)
                    )
            ...
            accuracy = (
                (total_count * len(data.columns) - error_count) / (total_count * len(data.columns))
                if total_count > 0
                else 0.0
            )
            return accuracy
    ...
    def _calculate_quality_score(self, metrics: Dict) -> float:
        """计算综合数据质量得分（百分比）"""
        weights = {"completeness": 0.25, "accuracy": 0.3, "timeliness": 0.25, "consistency_rate": 0.2}
        ...
        if "completeness" in self.metrics:
            if data is not None and not data.empty:
                ...
                if completeness < self.quality_rules["completeness_threshold"]:
                    issues.append(
                        f"数据完整度不足：{completeness:.2%}，阈值：{self.quality_rules['completeness_threshold']:.2%}"
                    )
                ...
                if "completeness_rate" in completeness_result["metrics"]:
                    metrics["completeness"] = completeness_result["metrics"]["completeness_rate"]
        ...
        if "accuracy" in self.metrics:
            if data is not None and not data.empty:
                ...
                if accuracy < self.quality_rules["accuracy_threshold"]:
                    issues.append(
                        f"数据准确率不足：{accuracy:.2%}，阈值：{self.quality_rules['accuracy_threshold']:.2%}"
                    )
                    overall_success = False
                    overall_level = max(overall_level, AlertLevel.WARNING)
            else:
                ...
                if "accuracy_rate" in accuracy_result["metrics"]:
                    metrics["accuracy"] = accuracy_result["metrics"]["accuracy_rate"]
