...
class MonitorResult:
    ...
    @classmethod
    def failure(
        cls, monitor_name: str, alert_level: AlertLevel = AlertLevel.ERROR, metrics: Dict = None, message: str = ""
    ) -> "MonitorResult":
        """创建失败的监控结果"""
        return cls(
            monitor_name=monitor_name,
...
class BaseMonitor(ABC):
    ...
    def __init__(
        ...
        self.alert_threshold = consecutive_failures_threshold or self.config.get(
            "alert_threshold", 3
        )  # 连续失败N次告警
    ...
    def run(self) -> MonitorResult:
        ...
        """
        if not self.enabled:
            return MonitorResult.success(monitor_name=self.name, message="监控已禁用")
        ...
        try:
            ...
            return MonitorResult.failure(
                monitor_name=self.name, alert_level=AlertLevel.ERROR, message=f"监控执行异常：{str(e)}"
            )
