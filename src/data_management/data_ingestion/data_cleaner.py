    ...
    def _detect_and_handle_outliers(self, df: pd.DataFrame, data_type: str) -> pd.DataFrame:
        ...
        try:
            ...
            def process_group(group):
                ...
                # 计算价格涨跌幅
                group['price_change'] = group['close'].pct_change().abs() if data_type in ['daily', 'minute'] else group['price'].pct_change().abs()
                ...
                group.loc[price_outlier, 'close' if data_type in ['daily', 'minute'] else 'price'] = group['close' if data_type in ['daily', 'minute'] else 'price'].shift(1)
