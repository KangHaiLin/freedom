...
class BatchProcessor(BaseProcessor):
    ...
    def __init__(self, config: Dict = None, chunk_size: int = None, max_workers: int = None, enable_parallel: bool = None):
        ...
        self.enable_parallel = enable_parallel if enable_parallel is not None else self.config.get('enable_parallel', True)
    ...
    def process_by_stock(
        ...
        **kwargs
        ...
        if self.enable_parallel and len(groups) > 1:
            with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                future_to_group = {
                    executor.submit(processor, group): group
                    for group in groups
                }
