...
# 测试配置
TEST_CONFIG = {
    "postgresql": {"host": "localhost", "port": 5433, "database": "stock_test", "user": "test", "password": "test123"},
    "clickhouse": {"host": "localhost", "port": 9000, "database": "stock_test", "user": "test", "password": "test123"},
...
def test_clickhouse_write(batch_size: int = 50000) -> Dict:
    ...
    start_time = time.time()
    conn.execute("INSERT INTO market_data (symbol, time, open, high, low, close, volume, amount) VALUES", rows)
    elapsed = time.time() - start_time
