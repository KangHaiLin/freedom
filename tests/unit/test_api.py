...
from user_interface.backend.dependencies import get_current_user, verify_admin_role, verify_api_key
...
class TestMarketDataAPI:
    ...
    def test_get_realtime_quote(self, mock_query_manager):
        ...
        mock_result.to_dict.return_value = {
            "data": [{"stock_code": "000001.SZ", "price": 10.0, "volume": 1000, "time": datetime.now().isoformat()}]
        }
        mock_result.total = 1
        ...
        response = client.get("/api/v1/market/realtime?stock_codes=000001.SZ&stock_codes=600000.SH", headers=headers)
        assert response.status_code == 200
        data = response.json()
    ...
    def test_get_daily_quote(self, mock_query_manager):
        ...
        mock_result.to_dict.return_value = {
            "data": [
                {"trade_date": "2023-01-01", "open": 10.0, "high": 10.5, "low": 9.5, "close": 10.2, "volume": 100000}
            ]
        }
        mock_result.total = 1
        mock_result.query_time = 0.1
        ...
        response = client.get(
            f"/api/v1/market/daily?stock_codes=000001.SZ&start_date={start_date}&end_date={end_date}", headers=headers
        )
        assert response.status_code == 200
        data = response.json()
        assert data["code"] == 200
...
class TestFundamentalDataAPI:
    ...
    def test_get_financial_indicators(self, mock_query_manager):
        ...
        mock_result.success = True
        mock_result.to_dict.return_value = {
            "data": [{"stock_code": "000001.SZ", "pe": 10.5, "pb": 1.2, "roe": 15.3, "market_cap": 100000000000}]
        }
        mock_result.total = 1
        mock_result.query_time = 0.1
        ...
        headers = {"X-API-Key": settings.API_KEYS[0]}
        response = client.get("/api/v1/fundamental/financial_indicator?stock_codes=000001.SZ", headers=headers)
        assert response.status_code == 200
        data = response.json()
        assert data["code"] == 200
...
class TestMonitorAPI:
    ...
    def test_get_monitor_status(self, mock_monitor_manager):
        ...
        mock_monitor_manager.get_monitor_status.return_value = {
            ...
            "monitors": [{"name": "data_quality", "status": "ok"}, {"name": "collection", "status": "ok"}],
        }
