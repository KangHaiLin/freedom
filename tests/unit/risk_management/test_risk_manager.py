...
def test_compliance_check_t1():
    ...
    rm = RiskManager()
    result = rm.compliance_check_t1(available_quantity=1000, sell_quantity=500, today_bought=300)
    assert result["passed"] is True

    result = rm.compliance_check_t1(available_quantity=1000, sell_quantity=800, today_bought=300)
    assert result["passed"] is False
